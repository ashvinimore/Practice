        ...
        for tic,co in zip(ticker,COID):
            ...
            try:
                for row in range(2, max_row):
                        ...
                        # # break
                        dwid = [362,363,364,365]
                        try:
                            #
                            for dwhid in dwid:
                                selectQuery  = "select DWCoID from StocksPeriodSchemaNew where DWCoID = %s and DWID = %s and ReportedDateID = %s"%(coid,dwhid,reporteddate)
                                cursor.execute(selectQuery)
                                res = cursor.fetchall()
                                print(res)

                                if res == []:
                                    insertQuery = "INSERT INTO StocksPeriodSchemaNew(DWID,ReportedDateID,DWCoID) VALUES (%s,%s,%s)" % (
                                     dwhid,reporteddate,coid)
                                    cursor.execute(insertQuery)
                                else:
                                    print('else')
                                    # updatequery = "Update StocksPeriodSchemaNew set SID362 = %s,SID363 = %s,SID364 = %s,SID365 = %s where  DWCoID = %s and PeriodID = %s and ReportedDateID = %s"\
