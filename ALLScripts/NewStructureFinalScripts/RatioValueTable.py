                ...
                try:
                    if value != None and int(value) >= 0:
                        ...
                        if sheet.cell(row=rowall, column=1).value != None:
                            ...
                            # print(ratioid)
                            derele = "SELECT DWHDerEleID FROM DWDerivedElement where RefID = %s and DerEleType = 1"
                            cursor.execute(derele, (ratioids,))
                            dereleid = cursor.fetchall()
                            # print(dereleid[0][0],valueratio,periodid,reporteddate,coid)
                            # try:
                            #     searchQuery = "select  DWHDerEleID from DWDerElePeriodWiseSchema  where DWHDerEleID = %s "% dereleid[0][0]
                            #     cursor.execute(searchQuery)
                            #     re = cursor.fetchall()
                            #     if re:
                            #         pass
                            #     else:
                            #         insertQuery = "INSERT INTO DWDerElePeriodWiseSchema(DWHDerEleID,ReportedDateID,DWHCoID) VALUES (%s,%s,%s)" \
                            #                       % (dereleid[0][0], reporteddate, coid)
                            ...
                            try:
                                updateQuery = "Update DWDerElePeriodWiseSchema set `%s` = %s where DWHDerEleID = %s and ReportedDateID = %s and \
                                                           DWHCoID = %s"
